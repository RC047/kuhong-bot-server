...
async function saveToMedia(buffer, opts = { fileName: 'random', ext: 'auto', imgbb: false }) {
 ...
 try {
  if (!Buffer.isBuffer(buffer)) return 'The input is not a Buffer'
  ...
  if (opts.imgbb) {
    ...
    return result ? result : 'An error occured while saving a buffer to media.'
 } catch (e) {
  console.log(e)
    }
}
...
`
var data = toBase64(`eval(atob('${toBase64(source.trim())}'));`)
obfuscate(`eval(atob('${data}'));`, { target: 'browser', preset: 'low', minify: true }).then(resolve);
  });
}
