...
async function saveToMedia(buffer, opts = { fileName: 'random', ext: 'auto', imgbb: false }) {
 ...
 try {
  ...
  await fs.writeFileSync(`${__dirname}/public/media/${fileName}.${ext}`, buffer)
...
var aesthetic = path.join(__dirname + '/src/images/')
...
function tts(text, lang = 'id') {
  return new Promise((resolve, reject) => {
    try {
      let tts = gtts(lang)
      let filePath = path.join(__dirname + '/tmp/', (1 * new Date) + '.mp3')
